...
from sklearn.metrics import roc_auc_score, roc_curve, balanced_accuracy_score, accuracy_score, precision_score, recall_score, f1_score
import utils
...
def calc_auc_and_acc(base_dir, heuristic_threshold, dataset_name='naturalquestion', embedding_model=None, threshold=0.0):
    ...
    for f in os.listdir(current_dir):
        ...
        max_sample = 3000 if dataset_name != 'commonsenseqa' else max(sample_sizes)
        ...
        with open(file_path, 'rb') as handle:
            ...
            for i, res in tqdm(enumerate(results)):
                ...
                reference_answers = res.get('reference_answers', [res['original_answer']])
                
                answer_embedding = embedding_model.submit_embedding_request(predicted_answer)
                max_similarity = -1
                
                for ref_answer in reference_answers:
                    ref_embedding = embedding_model.submit_embedding_request(ref_answer)
                    cos_sim = util.cos_sim(answer_embedding, ref_embedding).item()
                    if cos_sim > max_similarity:
                        max_similarity = cos_sim
                
                pred_best_scores.append(max_similarity)

            calc_metrics(gt, pred_best_scores, threshold)
